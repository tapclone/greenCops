  ...
  swal({
    ...
    if (willDelete) {
      try {
       ...
       await axios.delete(`/api/admin/deleteClients/${id}`,config).then(res=>{
        setLoading(!loading)
        }).catch(err=>{
          console.log(err);
        })
...
return (
    <div className='tableMain'  style={{marginLeft:'10%',marginTop:'3%',overflow:'scroll'}}>
      <button className='addBtn' style={{marginLeft:'70%',marginBottom:'1%'}} onClick={handleOpen}>Add Client</button>
